...
Community listing assets live in `assets/` and are generated by `python3 assets/render.py` (needs
Pillow); the aesthetic behind them is written down in [`assets/philosophy.md`](assets/philosophy.md).
The two typefaces it sets — SUSE and Geist Mono, both OFL — are vendored in `assets/fonts/` with
their licences, so the render reproduces from a clean clone. The publishing checklist and the
listing copy are in [`docs/PUBLISHING.md`](docs/PUBLISHING.md).
